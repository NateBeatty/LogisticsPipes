...
public abstract class LPTravelingItem {
    ...
    public static final class LPTravelingItemServer extends LPTravelingItem implements IRoutedItem {
        ...
        @Override
        public void setAdditionalSourceInformation(IAdditionalTargetInformation newSourceInfo) {
            info.sourceInfo = newSourceInfo;
        }

        @Override
        public IAdditionalTargetInformation getAdditionalSourceInformation() {
            return info.sourceInfo;
        }
        ...
        public void clearDestination() {
            if (info.destinationint >= 0) {
                itemWasLost();
                System.out.println("Tx35: " + info.destinationint);
            ...
            if (info.destinationint >= 0 && SimpleServiceLocator.routerManager.isRouter(info.destinationint)) {
                ...
                if (destinationRouter.getPipe() != null) {
                    if (!(info.targetInfo instanceof ModuleCrafter.CraftingChassieInformation)) {
                        System.out.println(
                                "Rerouting: " + info.getItem().getStackSize() + " " + info.getItem().getFriendlyName());
                    ...
                    if (destinationRouter.getPipe() instanceof IRequireReliableTransport) {
                        ...
                        System.out.println("Tx32: " + destinationRouter.getPipe().getRouterId());
                    }
        ...
        public void setSourceRouterID(int id) {
            info.sourceRouterID = id;
        }

        @Override
        public int getSourceRouterID() {
            return info.sourceRouterID;
        }
