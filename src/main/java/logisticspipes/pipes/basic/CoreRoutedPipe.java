...
public abstract class CoreRoutedPipe extends CoreUnroutedPipe
    ...
    private Map<ItemIdentifier, Integer> pendingRouting = new HashMap();
    ...
    public void queueRoutedItem(IRoutedItem routedItem, ForgeDirection from, ItemSendMode mode) {
        ...
        // System.out.println("Tx11"); //THIS

        /*
         * UpdatePendingRouting( routedItem.getItemIdentifierStack().getItem(), -1 *
         * routedItem.getItemIdentifierStack().getStackSize(), routedItem.getDestination()); sendQueueChanged(false);
         */
    }
    ...
    private void notifyOfSend(ItemRoutingInformation routedItem) {
        ...
        UpdatePendingRouting(
                routedItem.getItem().getItem(),
                routedItem.getItem().getStackSize() * -1,
                routedItem.destinationint);
    ...
    public IRoutedItem sendStack(ItemStack stack, int destination, ItemSendMode mode,
        ...
        // System.out.println("Tx13"); //THIS
        IRoutedItem itemToSend = SimpleServiceLocator.routedItemHelper.createNewTravelItem(stack);
        itemToSend.setDestination(destination);
        itemToSend.setSourceRouterID(getRouterId());
        itemToSend.setTransportMode(TransportMode.Active);
        itemToSend.setAdditionalTargetInformation(info);

        // UpdatePendingRouting(itemToSend.getItemIdentifierStack().getItem(), -1 * stack.stackSize, destination);
        queueRoutedItem(itemToSend, getPointedOrientation(), mode);
        return itemToSend;
    }

    public void UpdatePendingRouting(ItemIdentifier stack, int stackSize, int destinationRouterID) {
        IRouter r = SimpleServiceLocator.routerManager.getRouter(destinationRouterID);
        System.out.println("Updating routing table");
        if (r.getPipe() instanceof CoreRoutedPipe) {
            int oldAmount = r.getPipe().getPendingRouting(stack);
            r.getPipe().setPendingRouting(stack, Math.max(oldAmount + stackSize, 0));
        }
    }

    public int getPendingRouting(ItemIdentifier item) {
        return pendingRouting.getOrDefault(item, 0);
    }

    public void setPendingRouting(ItemIdentifier item, int amount) {
        if (amount == 0 && pendingRouting.containsKey(item)) {
            pendingRouting.remove(item);
        } else {
            pendingRouting.put(item, amount);
        }
    }
