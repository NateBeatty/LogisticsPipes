        ...
        if (filter == null) {
            filter = new IFilter() {
                ...
                public boolean isFilteredItem(ItemIdentifier item) {
                    if (FluidIdentifier.get(item) != null) {
                        FluidResource testFluid = new FluidResource(FluidIdentifier.get(item), 0, null);
                        return isFilteredItem(testFluid);
                    }
                ...
                public boolean isFilteredItem(IResource resultItem) {
                    for (Pair<ItemIdentifierStack, Integer> pair : inv) {
                        ...
                        } else if (!(resultItem instanceof FluidResource)
                                && resultItem.matches(stack.getItem(), IResource.MatchSettings.NORMAL)) {
                                    return true;
                                } else
                            if (resultItem instanceof FluidResource && FluidIdentifier.get(stack.getItem())
                                    == ((FluidResource) resultItem).getFluid()) {
                                        return true;
                                    }
                    }
                    return false;
