...
public class PipeItemsProviderLogistics extends CoreRoutedPipe implements IProvideItems, IHeadUpDisplayRendererProvider,
    ...
    private int sendStack(ItemIdentifierStack stack, int maxCount, int destination, IAdditionalTargetInformation info) {
        ...
        for (AdjacentTile tile : wUtil.getAdjacentTileEntities(true)) {
             ...
             * routedItem.getItemIdentifierStack().getStackSize(), routedItem.getDestination());
             */
            return sent;
        }
        _orderManager.sendFailed();
    ...
    public void enabledUpdateEntity() {
        ...
        while (itemsleft > 0 && stacksleft > 0
            ...
            /*
             * UpdatePendingRouting( order.getResource().getItem(), -1 * itemsleft,
             * order.getRouter().getPipe().getRouterId());
             */
